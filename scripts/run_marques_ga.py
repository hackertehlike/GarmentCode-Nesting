"""
Run GA on nesting-assets/marques.json 10 times with specific overrides.
...
def main(reps: int = 10):
    ...
    # Resolve marques.json path relative to repo root
    repo_root = Path(__file__).resolve().parents[1]
    pattern_path = repo_root / "nesting-assets" / "marques.json"
    if not pattern_path.exists():
        raise FileNotFoundError(f"Pattern file not found: {pattern_path}")

    # Create a top-level results folder for these runs
    base_out = repo_root / "results" / "marques_experiment"
...
if __name__ == "__main__":
    ...
    reps_str = os.getenv("MARQUES_GA_REPS", "10")
    try:
        reps = int(reps_str)
    except ValueError:
        reps = 10
