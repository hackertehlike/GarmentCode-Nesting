"""
Run GA on nesting-assets/shirts.json 10 times with specific overrides.

Overrides:
- allowed_rotations: [0, 180]
- population_weights: { elites: 0.25, offspring: 0, mutants: 0.5, randoms: 0.25 }
- mutation_weights: { rotate: 1, swap: 1, inversion: 1, insertion: 1, scramble: 1, split: 0, design_params: 0 }
- container_width_cm: 100
- container_height_cm: 40
...
from nesting.run_experiments import run_ga_on_patterns
...
def main(reps: int = 10):
    ...
    overrides = dict(
        ...
        container_height_cm=79,
        seam_allowance_cm=0.001,
    )
    config.load_config(**overrides)

    # Resolve shirts.json path relative to repo root
    repo_root = Path(__file__).resolve().parents[1]
    pattern_path = repo_root / "nesting-assets" / "trousers.json"
    if not pattern_path.exists():
        raise FileNotFoundError(f"Pattern file not found: {pattern_path}")

    # Create a top-level results folder for these runs
    base_out = repo_root / "results" / "trousers_experiment"
...
if __name__ == "__main__":
    ...
    reps_str = os.getenv("SHIRTS_GA_REPS", "10")
    try:
        reps = int(reps_str)
    except ValueError:
        reps = 10
