"""
Run GA on nesting-assets/dagli.json 10 times with specific overrides.
...
def main(reps: int = 10):
    ...
    pattern_path = repo_root / "nesting-assets" / "dagli.json"
    if not pattern_path.exists():
        raise FileNotFoundError(f"Pattern file not found: {pattern_path}")

    # Create a top-level results folder for these runs
    base_out = repo_root / "results" / "shirts_experiment"
