...
import numpy as np                             # ← Added: NumPy for vectorized ops
import pandas as pd
...
class Evolution:
    ...
    def __init__(
        ...
        # Population‐weight splits (elites, offspring, mutants, randoms)
        ...
        self.remainder   = (
            population_size 
            - (self.n_elites + self.n_offspring + self.n_mutants + self.n_randoms)
        )
        ...
        # ── metrics ────────────────────────────────────────────
        # We now accumulate fitness values directly via NumPy arrays
        # rather than pure‐Python sums. Storing histories in Python lists
        # of NumPy arrays is acceptable here.
        self.avg_child_fitnesses: list[float] = []
        self.best_fitness_history: list[float] = []
        self.delta_best: list[float] = []
        self.pop_fitness_history: list[np.ndarray] = []
        self.mean_offspring_gain: list[float] = []
        self.mean_mutant_gain: list[float] = []
        self.mutation_perf: dict[str, list[float]] = {}

        # Buffers for CSV‐output
        ts            = time.strftime("%Y%m%d_%H%M%S")
        log_dir = f"{config.SAVE_LOGS_PATH}_{ts}"
        ...
        self._swarm_gen_list    : list[int]   = []
        self._swarm_fit_list    : list[float] = []
        self._swarm_origin_list : list[str]   = []
    ...
    def _generate_random_chromosome(self) -> Chromosome:
        ...
        if config.VERBOSE:
            self._log(f"Random chromosome generated with {len(chrom.genes)} pieces.")

        # Randomize rotations
        for piece in chrom.genes:
            rotation = random.choice([0, 90, 180, 270])
            if config.VERBOSE:
                self._log(f"Random rotation for piece {piece.id}: {rotation}°")
            if rotation != 0:
    ...
    def generate_population(self) -> None:
        ...
        fitness_array = np.fromiter(
            (chrom.fitness for chrom in self.population),
            dtype=float,
            count=len(self.population)
        )
        self.pop_fitness_history.append(fitness_array)
    ...
    def next_generation(self) -> None:
        ...
        elite_fits = np.fromiter(
            (c.fitness for c in old_elite),
            dtype=float,
            count=len(old_elite)
        ) if old_elite else np.array([], dtype=float)
        avg_elite = float(elite_fits.mean()) if elite_fits.size > 0 else 0.0

        offspring, mutants, randoms = [], [], []
        off_gains, mut_gains = [], []

        # ── Offsprings ────────────────────────────────────────────────
        self._log("Submitting offspring batch…")
        with ThreadPoolExecutor() as offspring_executor:
            ...
            for _ in range(self.n_offspring):
                ...
                offspring_futures[fut] = "offspring"

            self._log("Collecting offspring results…")
            for fut in as_completed(offspring_futures):
        ...
        self._log(f"Collected {len(offspring)} offspring.")

        # ── Mutants ─────────────────────────────────────────────────
        self._log("Submitting mutant batch…")
        with ThreadPoolExecutor() as mutant_executor:
            ...
            for _ in range(self.n_mutants):
                ...
                mutant_futures[fut] = "mutant"

            self._log("Collecting mutant results…")
        ...
        self._log(f"Collected {len(mutants)} mutants.")

        # ── Randoms ─────────────────────────────────────────────────
        self._log("Submitting random batch…")
        with ThreadPoolExecutor() as random_executor:
            ...
            for _ in range(self.n_randoms):
                ...
                random_futures[fut] = "random"

            self._log("Collecting random results…")
            for fut in as_completed(random_futures):
        ...
        self._log(f"Collected {len(randoms)} randoms.")

        # ── Logging summary for batch collection ────────────────────────────────
        self._log(
            f"Batch summary: {len(offspring)} offspring, "
        ...
        offspring_fits = (
            np.fromiter((c.fitness for c in offspring), dtype=float, count=len(offspring))
            if offspring else np.array([], dtype=float)
        )
        mutant_fits = (
            np.fromiter((c.fitness for c in mutants), dtype=float, count=len(mutants))
            if mutants else np.array([], dtype=float)
        )
        random_fits = (
            np.fromiter((c.fitness for c in randoms), dtype=float, count=len(randoms))
            if randoms else np.array([], dtype=float)
        )

        avg_off  = float(offspring_fits.mean()) if offspring_fits.size > 0 else 0.0
        avg_mut  = float(mutant_fits.mean())  if mutant_fits.size > 0 else 0.0
        avg_rand = float(random_fits.mean())  if random_fits.size > 0 else 0.0

        mean_offspring_gain = float(np.fromiter(off_gains, dtype=float).mean()) if off_gains else 0.0
        mean_mutant_gain    = float(np.fromiter(mut_gains,  dtype=float).mean()) if mut_gains else 0.0
        self.mean_offspring_gain.append(mean_offspring_gain)
        self.mean_mutant_gain.append(mean_mutant_gain)

        self._log(
            f"Avg offspring gain vs parent: {mean_offspring_gain:+.4f}; mutants: {mean_mutant_gain:+.4f}"
        )

        # Finalize population & record metrics
        ...
        self.pop_fitness_history.append(all_fits)

        self.generation += 1
        best = float(all_fits.max())
        prev_best = self.best_fitness_history[-1] if self.best_fitness_history else best
        delta = best - prev_best
        self.best_fitness_history.append(best)
        self.delta_best.append(delta)

        # Append swarm data to lists
        for c in self.population:
            self._swarm_gen_list.append(self.generation)
            self._swarm_fit_list.append(c.fitness)
            self._swarm_origin_list.append(c.origin or "unknown")
        ...
        row = {
            ...
            'avg_child_fitness': float(all_fits.mean()),
            'avg_elite': avg_elite,
            'avg_off':   avg_off,
            'avg_mut':   avg_mut,
            'avg_rand':  avg_rand,
        ...
        row_mut = {
            f"mut_gain_{k}": (
                float(np.fromiter(g_list, dtype=float).mean()) if g_list else 0.0
            ) for k, g_list in self.mutation_perf.items()
        }
        row.update(row_mut)
        self.mutation_perf.clear()
    ...
    def _flush_metrics(self) -> None:
        ...
        self._csv_fieldnames = sorted(all_keys)
    ...
    def update_plots(self) -> None:
        """
        Overwrite three PNGs in plots/:
        1) fitness_curves:      avg_off, avg_mut, avg_rand, best_fit vs generation
        2) mean_gains:          mean_offspring_gain, mean_mutant_gain vs generation
        3) mutation gains plot: one line per mutation operator (if any)
        4) swarm plot: scatter of fitness per generation, colored by origin
        """
        ...
        df_metrics = pd.DataFrame(self._all_metrics)
        self._log(f"Plotting {len(df_metrics)} generations of metrics.")

        plt.figure(figsize=(8, 5))
        for col in ['avg_off', 'avg_mut', 'avg_rand', 'best_fit']:
            plt.plot(df_metrics['generation'], df_metrics[col], label=col)
        ...
        for col in ['mean_offspring_gain', 'mean_mutant_gain']:
            plt.plot(df_metrics['generation'], df_metrics[col], marker='o', linestyle='--', label=col)
        plt.xlabel("Generation")
        ...
        # ——— Plot 3: mutation gains (if any) ———
        if self.n_mutants != 0:
            mut_cols = [c for c in df_metrics.columns if c.startswith("mut_gain_")]
            if mut_cols:
                plt.figure(figsize=(8, 5))
                for col in mut_cols:
                    plt.plot(
                        df_metrics['generation'],
                        df_metrics[col],
                        marker='o',
                        linestyle='-',
        ...
        # ——— Plot 4: swarm plot ———
        # Build a single DataFrame from the three lists (once per plotting call)
        df_swarm = pd.DataFrame({
            "generation": self._swarm_gen_list,
            "fitness":    self._swarm_fit_list,
            "origin":     self._swarm_origin_list
        })
