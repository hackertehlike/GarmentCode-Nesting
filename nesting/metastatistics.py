...
class MetaStatistics:
    ...
    @staticmethod
    def _get_default_run_tag(length: int = 8) -> str:
        """Return a short, directory-safe run tag.

        Defaults to the first `length` characters of a UUID4 hex string.
        """
        length = max(4, min(int(length), 32))
    ...
    def ensure_master_files_exist(cls):
        ...
        # Create/upgrade master mutation statistics CSV to include run_tag, config_hash
        desired_mutation_header = [
            'timestamp', 'run_tag', 'config_hash', 'pattern_name', 'mutation_type',
        ...
        if not cls.MASTER_MUTATION_CSV_PATH.exists():
            with open(cls.MASTER_MUTATION_CSV_PATH, 'w', newline='') as f:
                csv.writer(f).writerow(desired_mutation_header)
        else:
            try:
                with open(cls.MASTER_MUTATION_CSV_PATH, 'r') as f:
                    first_line = f.readline().strip().split(',')
                if 'run_tag' not in first_line or 'config_hash' not in first_line:
                    legacy_path = cls.MASTER_MUTATION_CSV_PATH.with_name(
                        f"{cls.MASTER_MUTATION_CSV_PATH.stem}_legacy_{time.strftime('%Y%m%d_%H%M%S')}.csv"
                    )
                    cls.MASTER_MUTATION_CSV_PATH.rename(legacy_path)
                    print(f"Legacy {cls.MASTER_MUTATION_CSV_PATH.name} renamed to {legacy_path.name}; creating new file with extended header")
                    # Create new file with desired header
                    with open(cls.MASTER_MUTATION_CSV_PATH, 'w', newline='') as f:
                        csv.writer(f).writerow(desired_mutation_header)
                    # Attempt to backfill legacy data with empty tags
                    try:
                        legacy_df = pd.read_csv(legacy_path)
                        # Insert missing columns with empty values
                        legacy_df.insert(1, 'run_tag', '')
                        legacy_df.insert(2, 'config_hash', '')
                        # Ensure column order/superset
                        for col in desired_mutation_header:
                            if col not in legacy_df.columns:
                                legacy_df[col] = ''
                        legacy_df = legacy_df[desired_mutation_header]
                        legacy_df.to_csv(cls.MASTER_MUTATION_CSV_PATH, mode='a', header=False, index=False)
                        print(f"Backfilled legacy mutation stats from {legacy_path.name} into new schema")
                    except Exception as mig_e:
                        print(f"Failed to backfill legacy mutation stats: {mig_e}")
            except Exception as e:
                print(f"Could not inspect existing master mutation statistics header: {e}")
                
        # Create/upgrade raw mutation data CSV to include run_tag, config_hash
        desired_raw_header = [
            'timestamp', 'run_tag', 'config_hash', 'pattern_name', 'mutation_type', 'fitness_gain', 'generation', 'run_id'
        ]
        if not cls.MASTER_MUTATION_RAW_DATA_PATH.exists():
            with open(cls.MASTER_MUTATION_RAW_DATA_PATH, 'w', newline='') as f:
                csv.writer(f).writerow(desired_raw_header)
        else:
            try:
                with open(cls.MASTER_MUTATION_RAW_DATA_PATH, 'r') as f:
                    first_line = f.readline().strip().split(',')
                if 'run_tag' not in first_line or 'config_hash' not in first_line:
                    legacy_path = cls.MASTER_MUTATION_RAW_DATA_PATH.with_name(
                        f"{cls.MASTER_MUTATION_RAW_DATA_PATH.stem}_legacy_{time.strftime('%Y%m%d_%H%M%S')}.csv"
                    )
                    cls.MASTER_MUTATION_RAW_DATA_PATH.rename(legacy_path)
                    print(f"Legacy {cls.MASTER_MUTATION_RAW_DATA_PATH.name} renamed to {legacy_path.name}; creating new file with extended header")
                    # Create new file with desired header
                    with open(cls.MASTER_MUTATION_RAW_DATA_PATH, 'w', newline='') as f:
                        csv.writer(f).writerow(desired_raw_header)
                    # Attempt to backfill legacy data with empty tags
                    try:
                        ...
                        legacy_df.insert(1, 'run_tag', '')
                        legacy_df.insert(2, 'config_hash', '')
                        for col in desired_raw_header:
                            if col not in legacy_df.columns:
                                legacy_df[col] = ''
                        legacy_df = legacy_df[desired_raw_header]
                        legacy_df.to_csv(cls.MASTER_MUTATION_RAW_DATA_PATH, mode='a', header=False, index=False)
                        print(f"Backfilled legacy mutation raw data from {legacy_path.name} into new schema")
                    except Exception as mig_e:
                        print(f"Failed to backfill legacy mutation raw data: {mig_e}")
            except Exception as e:
                print(f"Could not inspect existing master mutation raw data header: {e}")
    ...
    def save_run_statistics(cls, evolution_instance, elapsed_time: float, run_tag: str = None, config_hash: str = None):
        ...
        # Also save mutation statistics (carry tags forward)
        #cls._save_mutation_statistics(evolution_instance, run_tag=run_tag, config_hash=config_hash)
    ...
    #                 # Extract identifiers from path and optional run_info.json
    #                 run_dir = gen_file.parent
    #                 run_tag = run_dir.name
    #                 pattern_name = run_dir.parent.name
    #                 cfg_hash_from_path = run_dir.parent.parent.name if run_dir.parent.parent.name not in (pattern_name, 'runs') else ''

    #                 # Try run_info.json first for reliable metadata
    ...
    #                 df['pattern_name'] = info.get('pattern_name', pattern_name)
    #                 df['run_tag'] = info.get('run_tag', run_tag)
    #                 df['config_hash'] = info.get('config_hash', cfg_hash_from_path)
    #                 df['run_id'] = f"{df['pattern_name'].iloc[0]}_{df['run_tag'].iloc[0]}_{gen_file.stat().st_mtime}"
    ...
    #             # Group by available identifiers (Generation is required; run_tag/config_hash are optional)
    #             group_cols = ['Generation']
    #             for extra in ['run_tag', 'config_hash']:
    #                 if extra in combined_df.columns:
    #                     group_cols.append(extra)

    #             avg_by_generation = combined_df.groupby(group_cols).agg({
