...
from nesting.core.layout import Piece, LayoutView
...
from nesting.data.panel_mapping import affected_panels
...
class Operators:
    ...
    ) -> Tuple[List[Piece], Optional[JsonDict], bool, Optional[Tuple[str, Any, Any]]]:
        """
        Mutate design parameters and return updated pieces, params, success status, and change info.

        Returns:
            (new_pieces, new_design_params, success, change_info)
            where change_info is (param_path, old_val, new_val) on success, None on failure.
        """
        if not (design_params and body_params):
            return pieces, design_params, False, None
            ...
            if success:
                ...
                if fitness_fn:
                    ...
                    if new_fitness != original_fitness:
                        return updated_pieces, current_design_params, True, (param, old_val, new_val)
                else:
...
def _apply_design_param_change(
    ...
    try:
        ...
        from nesting.data.panel_mapping import select_genes
